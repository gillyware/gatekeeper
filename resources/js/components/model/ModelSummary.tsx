...
import { type ConfiguredModel, type ModelEntitySupport, type VerbosePermission } from '@/types/api/model';
...
function EffectivePermissions({ effectivePermissions, language }: EffectivePermissionsProps) {
    ...
    const toggleAll = () => {
        const next = Object.fromEntries(effectivePermissions.map((p) => [p.name, !allOpen]));
        setOpenStates(next);
    };
